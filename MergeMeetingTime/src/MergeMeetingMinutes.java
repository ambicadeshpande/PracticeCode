...
public class MergeMeetingMinutes {
    ...
    public static List<Meeting> mergeRanges(List<Meeting> meetings) {
        ...
        //make a copy so we do not destroy the input
        ...
        // first element as the merged meeting
        mergedMeetings.add(sortedMeeting.get(0));

        for (Meeting meeting: sortedMeeting) {

            //get last merged meeting
            Meeting lastMergedMeeting = (mergedMeetings.get(mergedMeetings.size() - 1));

            if (meeting.getStartTime() <= lastMergedMeeting.getEndTime()){
                lastMergedMeeting.setEndTime(Math.max(lastMergedMeeting.getEndTime(),
                        meeting.getEndTime()));
            } else {
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(MergeMeetingMinutes.class);
